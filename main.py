        ...
        if os.path.exists(pwfile):
            with open(pwfile) as f:
                if message.text.strip() == f.read().strip():
                    ...
                    save_users(users)
        return bot.send_message(message.chat.id, "✅ Login successful!")
/start Start with।")
...
def ask_auth(message, field, cmd):
    bot.send_message(message.chat.id, f"🔐 {cmd} ব্যবহারের জন্য আবার Enter password:")
...
def rename_file(message, old, new):
    for root, _, files in os.walk(FILES_FOLDER):
        if old in files:
            ...
            bot.reply_to(message, "✅ Rename successful")
            return
